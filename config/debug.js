        ...
        try {
            ...
            try {
                if (!fs.existsSync(path.resolve(__dirname, `${debug_folder}res`))) {
                    fs.symlinkSync(path.resolve(__dirname, '../src/template-res'), path.resolve(__dirname, `${debug_folder}res`))
                }
            } catch (err) {
                console.warn('Failed to use symlink, fallback to dir copy');
                utils.copyDir(path.resolve(__dirname, '../src/template-res'), path.resolve(__dirname, `${debug_folder}res`))
            }
